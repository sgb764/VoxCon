...
class User{
  ...
  public function __construct(){
    ...
    if($this->isLogged()){
      ...
      if($this->_db->get('active_state', 'users', array('id'=>$this->data()->id))->first()->active_state === '0'){
    ...
    if($this->isLogged() && $this->data()->id !== $id){
      if(!$this->isFollowed($id)){
        ...
        if($this->_db->delete('followers', array(
          ...
          if($this->getNotification($user_idd, $post_idd, $type)){
 ...
 //Any posts?
  ...
  public function sendNotification($user_id, $post_id, $type, $body){
    if($this->isLogged()){
        ...
        if(
          ...
          if($clearred[1]){
            foreach($clearred[1] as $mention){
              ...
              $json = JSON_encode($body);
              $this->sendNotification($mention, $commentId, 5, $json);
            }
          }
